...
/** Pull the percentiles for `wanted` keys out of a radar value array. */
function pickPercentiles(
  keys: readonly string[], values: number[], wanted: readonly string[],
): number[] {
  return wanted.map((k) => {
    const i = keys.indexOf(k);
    return i >= 0 ? values[i] : NaN;
  });
}

// ---------------------------------------------------------------- QB
/** OVR inputs: quality only — no aDOT (style) and no dropbacks/game (volume). */
const QB_OVR_KEYS = ["epa_per_db", "cpoe", "success_rate", "inv_int_pct", "rush_epa"] as const;
...
export function buildQBCardData(
  ...
  const ovr = ovrFrom(
    pickPercentiles(QB_RADAR_KEYS, radarValues, QB_OVR_KEYS),
    eligible && pool.length > 0,
  );

  const intPct = me.attempts > 0 ? (me.interceptions / me.attempts) * 100 : NaN;
  const pct = (key: (typeof QB_RADAR_KEYS)[number]) => radarValues[QB_RADAR_KEYS.indexOf(key)];
...
const WR_OVR_RADAR_KEYS = ["epa_per_target", "croe", "yards_per_route_run"] as const;
...
export function buildWRCardData(
  ...
  const ovr = ovrFrom(
    [...pickPercentiles(WR_RADAR_KEYS, radarValues, WR_OVR_RADAR_KEYS), succPct],
    eligible && pool.length > 0,
  );

  const classify = me.position === "TE" ? classifyTE : classifyWR;
  const pct = (key: (typeof WR_RADAR_KEYS)[number]) => radarValues[WR_RADAR_KEYS.indexOf(key)];
  ...
  return {
    ...
    abilityRows: [
      ...
      { label: "CROE", raw: signedRate(me.croe, 1), percentile: pct("croe") },
      {
        label: "AIR YDS/TGT",
...
const RB_OVR_KEYS = ["epa_per_carry", "success_rate", "stuff_avoidance", "explosive_rate"] as const;
...
export function buildRBCardData(
  ...
  const ovr = ovrFrom(
    pickPercentiles(RB_RADAR_KEYS, radarValues, RB_OVR_KEYS),
    eligible && pool.length > 0,
  );

  const pct = (key: (typeof RB_RADAR_KEYS)[number]) => radarValues[RB_RADAR_KEYS.indexOf(key)];
