...
| Defensive / ST TDs | rows where `td_team == team` and either `posteam != team` or `kickoff_attempt == 1` (3 in week 1, all INT/fumble returns). The kickoff clause is needed because nflverse puts the **receiving** team in `posteam` on kickoffs (164 of 171 week-1 kickoff rows), so a kickoff-return TD would otherwise be missed; punts already carry the kicking team | 0 / 0 ✅ |
