...
| Fumbles lost | `fumble_lost == 1 and fumbled_1_team == posteam` — the same attribution as Turnovers above, never the bare flag. Attribution by `fumbled_1_team` is also what keeps `turnovers` from double-counting a pick the defence fumbles back | 0 / 2 ✅ |
