...
| Interceptions | `interception == 1` on this team's pass plays, 2-pt excluded | 0 / 0 ✅ |
| Fumbles lost | `fumble_lost == 1 and fumbled_1_team == posteam`, 2-pt excluded — the same attribution as Turnovers above, never the bare flag. Attribution by `fumbled_1_team` is also what keeps `turnovers` from double-counting a pick the defence fumbles back | 0 / 2 ✅ |
| First downs by type | `first_down_pass`, `first_down_rush`, `first_down_penalty` | 13/5/2 (sums to 20) · 11/10/5 (sums to 26) ✅ |
| 4th down | attempts and conversions, `no_play` excluded | 0-1 / 2-2 ✅ |
| 3rd / 4th down | attempts and conversions **excluding `no_play` rows** — the opposite of the efficiency filter. Including them gives HOU 8-of-18. A conversion is `first_down_rush` or `first_down_pass`; a penalty-only first down is not a conversion (6 team-games in 2026 wks 1–2). A spike (`qb_spike`) counts as a snap from scrimmage here and for red-zone trips | 3-9 / 7-16 ✅ |
| First downs (total) | `first_down_pass + first_down_rush + first_down_penalty` | 20 / 26 ✅ |
| Red zone trips / TDs | **drive-level**: a drive is a trip if a scrimmage snap or field-goal attempt starts at `yardline_100 < 20` (a snap from the 20 itself is not a trip); a score if that drive ends in a TD **scored by this team** (`td_team == team`), so a red-zone pick-six is not credited to the offence (no such case in week 1, so the golden test cannot catch it). Scrimmage plays and field-goal attempts only, 2-pt excluded (play-level counting gives BUF 6 trips because extra points snap from the 15) | 1-3 / 4-5 ✅ |
| Penalties / yards | `penalty == 1 and penalty_team == team` | 10-85 / 7-106 ✅ |
| Turnovers | `interception == 1` + (`fumble_lost == 1 and fumbled_1_team == posteam`), 2-pt excluded. **`fumble_lost` alone is wrong**: it flags the play, not who lost the ball. In 2025, 4 plays have an interception *and* a lost fumble where the defence fumbled and the offence recovered — 1 turnover, not 2 — and 16 more plain lost-fumble rows have `fumbled_1_team != posteam`, crediting the offence with a giveaway it never made. Season totals: bare flag 628 → this rule 610, correct in 543 of 544 team-games.<br><br>**Known limitation, 1 of 544:** `2025_14_PHI_LAC` has an interception *and* a genuinely separate lost fumble on the same snap, recorded only in `fumbled_2_team`, so this rule gives PHI 4 when 5 is correct. `fumbled_2_team` is not trustworthy enough to fix it — on `2025_17_SEA_CAR` it holds the *recovering* team — so this stays bounded and documented.<br><br>**Known limitation, muffed punts:** the rule is keyed by `posteam`, which on a punt is the *kicking* team, so a punt the *receiving* team muffs and loses (`2026_01_CHI_CAR`, the only week-1 case) is credited to nobody, although ESPN charges the receiver. Pinned as a strict xfail test, not patched.<br><br>**Do not "improve" the rule** by also requiring `fumble_recovery_n_team != posteam`: that drops genuine turnovers where nobody recovers (`2025_04_IND_LA`, a fumble out of the end zone for a touchback, has a null recovery team) and scores worse, 607<br><br>**`epa_lost_turnovers` ("What it cost them", below) is NOT 2-pt excluded** — it is computed on the efficiency set, which keeps two-point tries, so an intercepted or fumbled 2-point try still adds to that column even though it does not count as a turnover here | 0 / 2 ✅ |
...
| Defensive / ST TDs | rows where `td_team == team` and either `posteam != team` or `kickoff_attempt == 1` (3 in week 1, all INT/fumble returns). The kickoff clause is needed because nflverse puts the **receiving** team in `posteam` on kickoffs — all 171 week-1 kickoff rows carry the receiving team in `posteam` (the kicking team was identified two independent ways, by kicker id and from the play description, and both agree on all 171; 164 of them also have `posteam == return_team`, the other 7 simply have a null `return_team`) — so a kickoff-return TD would otherwise be missed; punts already carry the kicking team | 0 / 0 ✅ |
...
| EPA lost to penalties | **This team's own penalties only**: rows with `penalty == 1 and penalty_team == team`, the same rows as the Penalties stat. Then (sum of `epa` on those rows where this team had the ball) − (sum of `epa` on those rows where this team was on defense). Week 1: **BUF −8.52, HOU −9.69** — pin both in the golden test. Two ways to get it wrong: summing `epa` over the team's penalties without the sign flip gives BUF +2.39, because 6 of their 10 penalties came on defence where the EPA belongs to HOU; and running the formula over **every** penalty row in the game (both teams' flags) gives BUF +1.17 |
...
**Rendering:** no `generateStaticParams` — team pages enumerate from the static `NFL_TEAMS` constant with no DB read, but game ids would need one, and a Supabase blip would then fail every build. Pages are generated on demand with `revalidate = 3600`; `/api/revalidate` gains `/game` (layout). The sitemap lists played, covered games — noting that `app/sitemap.ts:10-14` swallows read errors (the documented exception), so a failed read there silently drops every game URL until the next rebuild.
...
1. **QB rushing EPA and success rate.** `qb_weekly_stats` stores only rushing attempts/yards/TDs, so Josh Allen's 5 carries and 2 rushing TDs show "—". Add `rush_epa_per_carry` and `rush_success_rate` (NUMERIC). `ensure_qb_weekly_stats_table` (`scripts/ingest.py:2393`) is `CREATE TABLE IF NOT EXISTS` only and cannot add columns, so this needs `ALTER TABLE … ADD COLUMN IF NOT EXISTS` in the style of `ensure_qb_season_stats_columns` (`:1546-1568`), plus the new columns in `upsert_qb_weekly_stats`'s `cols` list. **Compute both over exactly the plays `rush_attempts` counts:** designed runs (`designed_rushes`, `:2060`, kneels already excluded) **plus scrambles** (`scramble_plays`, `:2072`, keyed by `passer_player_id` after the scramble fill), which `:2085` adds into `rush_attempts`. Use the same non-null-`epa` rows the count uses, so the EPA and success rate describe the carries shown. Week 1: Allen 5 carries (4 runs + 1 scramble) → −0.46 EPA/carry, 40% success; Stroud 2 (1 run + 1 scramble) → +0.73, 50%. Designed runs alone would give Allen −1.17 and 25% over 4 plays, which no longer matches the 5 carries in his row. Test it with a synthetic QB who has one designed run and one scramble.
...
**Python (pytest):**
- made-up games covering every stored stat and the tricky cases: a 2-point try (kept in efficiency, excluded from targets and traditional counts), kneels, penalty-wiped plays, a strip-sack, **an interception the defence fumbles back** (1 turnover, not 2), **a defensive fumble the offence recovers** (not a giveaway), **a fumble the offence loses and recovers itself** (`fumble_lost == 0`, not a turnover), a lateral, a defensive TD, a safety, a drive with no scrimmage play, a team with zero pass or zero rush attempts (synthetic only — 0 of 544 2025 team-games), and a game with no plays yet
- **golden test** on real games: efficiency rows must match rbsdm exactly (§4's BUF–HOU numbers) and traditional rows the official ESPN box score — 20/26 first downs, 409/381 yards, **52/73 total plays, 7.9/5.2 yards per play**, 3-9 / 7-16 third down, 1-3 / 4-5 red zone, **2-11 / 3-17 sacks**, 10-85 / 7-106 penalties, 0/2 turnovers, 23:43 / 36:17 possession, and what-it-cost-them: EPA lost to penalties −8.52 / −9.69 (§4). Every stored traditional and what-it-cost-them column must appear in a golden assertion, or an error in it is invisible. Use **NO–DET** as the overtime case (TOP sums to 68:26, result derives correctly)
...
- if any test pins `2025_14_PHI_LAC`, the correct answer is **5 turnovers** and the hook "PHI turned it over 5 times (+3)"; the rule returns 4, so mark it a documented known limitation (§4), never a passing assertion of 4
