...
- **Commits:** stage files by name. End every commit message with the `Co-Authored-By:` line your own session's attribution reminder gives you — it names the model actually doing the work, so it differs between tasks. Use that line, not the generic one the commit commands below still carry, and not another task's.
