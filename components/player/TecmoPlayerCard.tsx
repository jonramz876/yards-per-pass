...
export default function TecmoPlayerCard({
  ...
  return (
    <div className="bg-white rounded-lg shadow overflow-hidden max-w-2xl lg:max-w-4xl mx-auto">
      {/* Team band */}
      <div
        className="flex justify-between px-4 py-2.5 lg:px-5 lg:py-3 text-[9px] sm:text-[10px] lg:text-xs font-[family-name:var(--font-pixel)] uppercase tracking-wide"
      ...
      <div className="p-4 lg:p-5">
        ...
        <div className="flex items-center gap-3">
          <JerseyAvatar
            ...
            playerName={data.playerName} size={64}
          />
          <div className="min-w-0">
            <div className="font-[family-name:var(--font-pixel)] text-[11px] sm:text-xs lg:text-base text-navy uppercase truncate">
              {jerseyNumber != null ? `${jerseyNumber}-` : ""}{data.playerName}
            </div>
            <div className="font-[family-name:var(--font-pixel)] text-[7px] sm:text-[8px] lg:text-[10px] text-gray-500 uppercase mt-1.5">
              {data.season} &middot; {data.games} GAMES{data.archetypeLabel ? ` · ${data.archetypeLabel}` : ""}
            </div>
          </div>
          <div
            className="ml-auto shrink-0 text-center rounded-md px-2.5 py-1.5 lg:px-3 lg:py-2 font-[family-name:var(--font-pixel)]"
            style={{ background: primaryColor, color: bandText }}
          >
            {/* Nullish coalescing, never a truthiness check: an OVR of 0 is a real score. */}
            <div data-ovr className="text-sm lg:text-xl">{data.ovr ?? EM_DASH}</div>
            <div className="text-[6px] lg:text-[8px]">OVR</div>
          </div>
        </div>

        {/* Basic stat grid */}
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5 lg:gap-2 mt-3">
          {data.statCells.map((c) => (
            <div key={c.label} className="bg-slate-50 border border-slate-200 rounded px-1 py-1.5 lg:py-2 text-center">
              <div className="font-[family-name:var(--font-pixel)] text-[6px] lg:text-[8px] text-gray-500 uppercase">{c.label}</div>
              <div className="text-sm lg:text-lg font-bold text-navy mt-0.5">{c.value}</div>
            </div>
          ))}
        </div>
          ...
          <div className="flex-1 w-full min-w-0">
            {data.abilityRows.map((r) => (
              <div key={r.label} className="flex items-center gap-2 mt-2">
                ...
                />
                <span className="font-[family-name:var(--font-pixel)] text-[7px] lg:text-[9px] text-slate-700 uppercase basis-1/3 shrink-0">
                  {r.label}
                </span>
                <span aria-hidden="true" className="flex-1 h-2 lg:h-2.5 bg-slate-200 rounded-full overflow-hidden min-w-0">
                  <span
                    data-bar-fill
                    className="block h-2 lg:h-2.5 rounded-full"
                    style={{
        ...
        <div className="font-[family-name:var(--font-pixel)] text-[6px] lg:text-[8px] text-slate-400 text-center mt-3 uppercase">
